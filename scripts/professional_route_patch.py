...
s=s.replace('const [active,setActive]=useState(getToolFromPath());','const [activeToolId,setActive]=useState(getToolFromPath());')
s=s.replace('setMeta(active)', 'setMeta(activeToolId)')
s=s.replace('[active]);', '[activeToolId]);')
s=s.replace('const current=tools.find(t=>t.id===active)||tools[0];', 'const current=tools.find(t=>t.id===activeToolId)||tools[0];')
s=s.replace('active={active===t.id}', 'active={activeToolId===t.id}')
s=s.replace('className={active===t.id?', 'className={activeToolId===t.id?')
s=s.replace('document.title=`${meta.title} — DevBox`;', 'document.title=toolId?`${meta.title} — DevBox`:"DevBox — Developer Toolkit";')
...
.quick svg:first-child{width:18px;height:18px;flex:0 0 18px}.quick svg:last-child{width:14px;height:14px;flex:0 0 14px}
.quick span{min-width:0;display:flex;flex-direction:column;gap:4px}.quick strong,.quick small{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.tool-icon{width:36px!important;height:36px!important;min-width:36px!important;max-width:36px!important;display:grid!important;place-items:center!important;border-radius:50%!important}.tool-icon svg{width:18px!important;height:18px!important;max-width:18px!important;max-height:18px!important}
.openapi-workbench svg{max-width:18px;max-height:18px}.openapi-layout,.openapi-spec,.openapi-explorer{min-width:0}.openapi-spec textarea{width:100%;max-width:100%}
.category-pills{overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none}.category-pills::-webkit-scrollbar{display:none}.content{min-width:0}
@media(max-width:1050px){.quick-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.quick-grid{grid-template-columns:1fr}.quick{height:70px}}
''')
imp='import "./professional-cleanup.css";'
if imp not in s:s=s.replace('import "./styles.css";', 'import "./styles.css";\n'+imp)
main.write_text(s)
print("professional registry, routing, active state and UI cleanup applied")
